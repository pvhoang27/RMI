//[Mã câu hỏi (qCode): dXqHfH4i].  Một chương trình (tạm gọi là RMI Server) cung cấp giao diện cho phép triệu gọi từ xa để xử lý dữ liệu nhị phân.
//Giao diện từ xa:
...
public class xor {
    public static void main(String[] args) throws Exception{
        String studentCode = "B22DCCN008",qCode = "dXqHfH4i";
        Registry rg = LocateRegistry.getRegistry("203.162.10.109");
        ByteService sv = (ByteService) rg.lookup("RMIByteService");
        byte[] a = sv.requestData(studentCode, qCode);
        System.out.println(a);
        byte[] goc = "PTIT".getBytes();
        byte[] moi = new byte[a.length];
        byte[] kq = new byte[a.length];
        
        for(int i = 0 ; i < a.length; i ++){
            moi[i] = goc[ i % goc.length];
        }
        for(int i = 0 ; i < a.length; i++){
            kq[i] = (byte) (a[i] ^ moi[i]) ;
        }
